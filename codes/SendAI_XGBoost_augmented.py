...
def main():
    ...
    target_count = int(max_class_count * 0.8)  # Target all'80% della classe più numerosa
    ...
    for idx, row in tqdm(df_train.iterrows(), total=len(df_train), desc="Augmenting tweets"):
        ...
        # Calcola quante augmentation servono per questa classe
        current_class_count = class_counts[row['label']]
        if current_class_count < target_count:
            # Calcola quante augmentation servono per raggiungere il target
            augmentations_needed = target_count - current_class_count
            # Limita il numero di augmentation per ogni esempio
            augmentations_per_example = min(3, augmentations_needed)
            
            # Generate augmented versions
            for _ in range(augmentations_per_example):
                augmented_versions = augmenter.augment_single_text(row['text'], enabled_augmentations)
                if len(augmented_versions) > 1:  # Se ci sono versioni augmentate
                    augmented_texts.append(augmented_versions[1])  # Prendi la prima versione augmentata
                    augmented_labels.append(row['label'])
    ...
    joblib.dump(xgb_model, 'sentiment_model_xgb_augmented.joblib')
    joblib.dump(scaler, 'scaler_xgb_augmented.joblib')
    joblib.dump(label_encoder, 'label_encoder_xgb_augmented.joblib')
